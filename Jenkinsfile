node {
	...
	stage('Test with Maven/PSQL') {
		...
		container('maven'){
			dir ("./${appName}") {
				sh ("mvn test -Dspring.profiles.active=prod -Dspring.datasource.url=jdbc:postgresql://postgres/${DB_NAME} -Dspring.datasource.username=${DB_USERNAME} -Dspring.datasource.password=${DB_PASSWORD}")
			}
		}
	}
